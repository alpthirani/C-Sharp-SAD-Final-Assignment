...
namespace BookStore
{
    public partial class Science_Fictionfrm : Form
    {
        private string category;
        public Science_Fictionfrm(string category)
        {
            InitializeComponent();
            this.category = category;
        }

        public Science_Fictionfrm() : this("Default Category")
        {
            // You can set a default category here if needed
        }

        public void Science_Fiction_Load(object sender, EventArgs e)
        {
        ...
        public void DisplayBooksByCategory(string category)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    ScienceFictionGridView.DataSource = dataTable;
                }
            }
        }
        ...
        {
            // when click the X Science Fiction from will close
            Application.Exit();
        }
